...
<meta charset="utf-8">
    ...
    <title>{{Helper::getSetting(1)->description}} - DIRECTOR - {{@$listStory->name}}</title>
    <meta property="og:url" content="{{@url('/')}}">
    <meta property="og:type" content="website">
    <meta property="og:title" content="{{Helper::getSetting(1)->description}} - DIRECTOR - {{@$listStory->name}}">
    <meta property="og:description" content="{{Helper::getSetting(2)->description}}">
    <meta property="og:image" content="{{@url(@$listStory->image_pc ? $listStory->image_pc : '/')}}">
    <!-- <meta property="fb:app_id" content=""> -->

    <meta name="description" content="This is the website description. Nice eh?">
 
    <!-- Google / Search Engine Tags -->
    <meta itemprop="name" content="{{Helper::getSetting(1)->description}} - DIRECTOR - {{@$listStory->name}}">
    <meta itemprop="description" content="{{Helper::getSetting(2)->description}}">
    <meta itemprop="image" content="{{@url(@$listStory->image_pc ? $listStory->image_pc : '/')}}">
   
    
    <!-- Twitter Meta Tags -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="{{Helper::getSetting(1)->description}} - DIRECTOR - {{@$listStory->name}}">
    <meta name="twitter:description" content="{{Helper::getSetting(2)->description}}">
    <meta name="twitter:image" content="{{@url(@$listStory->image_pc ? $listStory->image_pc : '/')}}">


    <meta name="keywords" content="{{Helper::getSetting(1)->description}} - DIRECTOR - {{@$listStory->name}}">
    ...
    <link rel="stylesheet" href="{{ asset('fe/css/detail-derector.css') }}">
    ...
    <div class="nk-wrap">
        <header>
            ...
            <div class="header_titleLogo">
                <a href="/director-{{$story->id}}.html" class="titleLogo">
                    {{@$story->description}}
                </a>
                <p class="contentLogo">{{$listStory->name}}</p>
            </div>
        ...
        <main >
            <div class="bg_style" style="background: rgba(0,0,0,0.4) ;">
                <div class="home_content_image">
                    <div class="logo-content-about-us mobile-display">
                        <h3 class="title_video_mobile">{{$listStory->name}}</h3>
                        @if($listStory->link_video != '')
                            <iframe src="{{$listStory->link_video}}"></iframe>
                        @elseif($listStory->link_youtube != '')
                            <iframe src="{{$listStory->link_youtube}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                        @else
                            <video controls class="video_detail">
                                <source src="{{@url(@$listStory->video ? $listStory->video : '/' )}}" type="video/mp4">
                            </video>
                        @endif
                    </div>
                    ...
                    <div class="controlContent">
                        <h3 class="title_video pc_display_title">{{$listStory->name}}</h3>
        ...
        $(document).ready(function() {
            if (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent) ||
                ...
                $('.menu_btn_toggle').on('click', function() {
                    ...
                    $(".logo-content-about-us.pc-display").animate({
                        width: "0px"
                    }, 500);
                    ...
                    $('header').addClass('hidden');
                    $('footer').addClass('hidden');
                    // $('.logo-content-about-us.pc-display').hide('slow');
                    // $('.content-about-us').attr("style",'width: calc(100% - 450px)');
                });
            ...
            $(document).mouseup(function(e) {
                var container = $(".box");
                if (!container.is(e.target) && container.has(e.target).length === 0 && $(".box").width() > 0) {
                    var widthWindows = $(window).width();
                    // $('.content-about-us').attr("style",'width: calc(50%)');
                    // $('.logo-content-about-us.pc-display').show('slow');
                    $(".box").animate({
                        width: "0px"
                    }, 500);
                    $(".controlContent").animate({
